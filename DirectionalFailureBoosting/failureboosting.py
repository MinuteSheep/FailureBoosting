...
def precalc(lwe_n, q, sd, B, m, name):
    ...
    plt.loglog(alpha, beta, base=2, color='r', label='no extra info')
    for i in alphabetalist[1:]:
        alphac, betac, ciphertexts = i
        plt.loglog(alphac, betac, base=2, color='b', label=str(ciphertexts) + ' ciphertext')
    ...
    plt.loglog(beta, [np.sqrt(a) * b**-1 for a, b in zip(alpha, beta)],
               base=2, color='r', label='no extra info')
    for i in alphabetalist[1:]:
        alphac, betac, ciphertexts = i
        plt.loglog(betac, [np.sqrt(a) * b**-1 for a, b in zip(alphac, betac)],
                   base=2, color='b', label=str(ciphertexts) + ' ciphertext')
...
def directionalFailureboosting(lwe_n, q, sd, B, m, name, limitedqueries=False, multitarget=False):
    ...
    #  Let's plot  #
    ################
    plt.loglog(ciphertextlist, worklist, label='work', base=2)
    if not multitarget:
        plt.loglog(ciphertextlist, workoldlist, label='work - traditional', base=2)
    plt.loglog(ciphertextlist, querylist, label='queries', base=2)
    if not multitarget:
        plt.loglog(ciphertextlist, queryoldlist, label='queries - traditional', base=2)
    ...
    plt.loglog(ciphertextlist, worklist, label='work', base=2)
    if not multitarget:
        plt.loglog(ciphertextlist, workoldlist, label='work - traditional', base=2, linestyle='dashed')
    ...
    plt.loglog(ciphertextlist, querylist, label='queries', base=2)
    if not multitarget:
        plt.loglog(ciphertextlist, queryoldlist, label='queries - traditional', base=2, linestyle='dashed')
    ...
    plt.rcParams.update({'font.size': 16})
    plt.loglog(ciphertextlist, worknextciphertext, label='work', base=2)
    plt.loglog(ciphertextlist, querynextciphertext, label='query', base=2)
